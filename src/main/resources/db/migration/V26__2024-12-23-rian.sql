...
INSERT INTO inteliagent.sync_tables
(ID, TABELA, ROTINA)
...
INSERT INTO inteliagent.sync_tables
(ID, TABELA, ROTINA)
VALUES(6, 'VISITA', 'SELECT ''(ID, PRIMEIRA_VISITA, SEGUNDA_VISITA, TIPO_VISITA, NUMERO_QUARTOS, CONTEM_AMOSTRA, CONTEM_TRATAMENTO, SITUACAO_REFERENCIA, SITUACAO, ID_PLANEJAMENTO, CEP, NOME, NUMERO, COMPLEMENTO)''
UNION
SELECT CONCAT(''(\\'''',ID,''\\'','', ''\\'''',PRIMEIRA_VISITA,''\\'','', ''\\'''',SEGUNDA_VISITA,''\\'','', ''\\'''',TIPO_VISITA,''\\'','', coalesce(NUMERO_QUARTOS,0),'','', ''\\'''',coalesce(CONTEM_AMOSTRA,''''),''\\'','', ''\\'''',coalesce(CONTEM_TRATAMENTO,''''),''\\'',\\'''', coalesce(SITUACAO_REFERENCIA,''''),''\\'','', coalesce(SITUACAO,0),'',\\'''', coalesce(ID_PLANEJAMENTO,''''),''\\'','', ''\\'''',coalesce(CEP,''''),''\\'','', ''\\'''',coalesce(NOME,''''),''\\'','', coalesce(NUMERO,0),'','', ''\\'''',coalesce(COMPLEMENTO, ''''),''\\'''','')'') from VISITA
where updated_at >= :DATA_ULTIMO_SINCRONISMO and updated_at < :DATA_ATUAL');